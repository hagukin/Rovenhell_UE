...
bool UActorPhysicsSyncComponent::IsActorInSyncWith(const FTransform& Transform)
{
	...
	for (uint32 i = 0; i < (uint32)MAX_PHYSICS_HISTORY_SIZE; ++i)
	{
		...
		if(IsValidPhysicsData(currentIndex, Transform))
		{
			return true;
		}
	...
	return IsValidPhysicsData(prev, Transform);
}
...
bool UActorPhysicsSyncComponent::IsValidPhysicsData(uint32 index, const FTransform& Transform)
{
