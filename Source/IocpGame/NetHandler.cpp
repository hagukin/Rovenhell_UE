...
bool NetSession::Connect(NetAddress connectAddr)
{
	...
	if (ret)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("NetHandler: 소켓 연결 성공 %s:%i"), *connectAddr.GetIp(), connectAddr.GetPort()));
		_peerAddr = connectAddr;
	}
	else
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("NetHandler: 소켓 연결 실패 %s:%i"), *connectAddr.GetIp(), connectAddr.GetPort()));
	}
	return ret;
}
